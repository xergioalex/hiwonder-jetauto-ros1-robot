...
def record_audio_push_to_talk():
    """
    Record audio using push-to-talk method.
    User presses ENTER to start recording, then presses ENTER again to stop.
    Returns the path to the saved WAV file, or None if recording failed.
    """
    if not AUDIO_AVAILABLE:
        print("Error: Audio libraries not available. Cannot record audio.")
        return None
    
    # Audio recording parameters
    sample_rate = 16000  # 16kHz for OpenAI STT
    channels = 1  # Mono
    ...
    def record_callback(indata, frames_count, time_info, status):
        ...
        if status:
            print("Recording status: {}".format(status))
        if recording:
    ...
    try:
        ...
        with sd.InputStream(samplerate=sample_rate, channels=channels, 
                           dtype=dtype, callback=record_callback, blocksize=1024):
            # Wait for second ENTER press to stop (this blocks)
            try:
                input()
            except (KeyboardInterrupt, EOFError):
            ...
            # Stop recording
            recording = False
            time.sleep(0.2)  # Small delay to ensure last frames are captured
        ...
        # Trim trailing silence (optional, but helpful)
        # Find last non-zero sample
        non_zero_indices = np.where(audio_data != 0)[0]
        if len(non_zero_indices) > 0:
            # Keep a small buffer at the end
            end_index = min(non_zero_indices[-1] + int(0.1 * sample_rate), len(audio_data))
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("Robot Voice Controller - Push-to-Talk Mode")
    print("=" * 70)
    print("You can control the robot in two ways:")
    print("  1. Type commands and press ENTER")
    print("  2. Press ENTER (twice) for voice input:")
    print("     - First ENTER: Start recording")
    print("     - Second ENTER: Stop recording")
    print("")
    ...
    try:
        while not should_exit:
            ...
            if not user_input and AUDIO_AVAILABLE:
                ...
                audio_file = record_audio_push_to_talk()
